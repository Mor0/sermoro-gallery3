...
class gallery_graphics_Core {
  ...
  static function resize($input_file, $output_file, $options, $item=null) {
    ...
    $w = escapeshellarg($options["width"]);
	  $h = escapeshellarg($options["height"]);
	  $d = module::get_var("gallery", "graphics_toolkit_path");
	  if($error = exec($d.'/convert '.escapeshellarg($input_file).' -coalesce -bordercolor White -border 0 -resize '.$w.'x'.$h.' -layers Optimize '.escapeshellarg($output_file))) {

	  } else {
	    $image = Image::factory($input_file)
	      ->resize($options["width"], $options["height"], $options["master"])
	      ->quality(module::get_var("gallery", "image_quality"));
	    if (graphics::can("sharpen")) { $image->sharpen(module::get_var("gallery", "image_sharpen")); }
        }
