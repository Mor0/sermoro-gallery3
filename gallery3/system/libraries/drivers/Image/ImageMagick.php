  ...
  // Directory that IM is installed in
	...
	public function process($image, $actions, $dir, $file, $render = FALSE, $background = NULL)
	{
		...
		if ($status = $this->execute($actions))
		{
			...
			$flatten = ' -flatten';
			$qua = ' -quality '.$quality.'% ';
			$com = escapeshellcmd($this->dir.'identify'.$this->ext).' '.$imagga;
			if ($error=exec($com, $skop)) 
			{
				$this->errors[] = $error;
			}
			if (sizeof($skop)>1)
			...
			$com = escapeshellcmd($this->dir.'convert'.$this->ext).' -background '.$this->background.$flatten.$qua.$this->cmd_image.' '.$this->new_image;
			if ($error = exec($com))
			{
				$this->errors[] = $error;
			}
	...
	public function resize($prop)
	{
		...
		$skop = array();
		$skop = '';
		$com = escapeshellcmd($this->dir.'identify'.$this->ext).' '.$this->cmd_image;
		if ($error=exec($com, $skop))
		{
			$this->errors[] = $error;
			return FALSE;
		...
		if (sizeof($skop)>1){
			if ($error=exec(escapeshellcmd($this->dir.'convert'.$this->ext).' '.$this->cmd_image.' -coalesce -bordercolor LightSteelBlue -border 0 -resize '.$dim.' -layers Optimize '.$this->cmd_image))
			{
				$this->errors[] = $error;
				return FALSE;
			}
